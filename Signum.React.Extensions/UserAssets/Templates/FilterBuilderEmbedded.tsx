...
import { QueryDescription, SubTokensOptions, isFilterGroupOptionParsed, FilterConditionOptionParsed, isList, FilterType, FilterGroupOptionParsed, PinnedFilter, PinnedFilterParsed, getFilterType, getFilterGroupUnifiedFilterType } from '@framework/FindOptions'
...
export default function FilterBuilderEmbedded(p: FilterBuilderEmbeddedProps) {
  ...
  function handleRenderValue(fc: RenderValueContext) {
    if (isFilterGroupOptionParsed(fc.filter)) {
      ...
      var format = (tr && f.filters.map(a => a.token!).distinctBy(a => a.format ?? "").onlyOrNull()?.format) ?? undefined;
      ...
      return <ValueLineOrExpression ctx={ctx} onChange={fc.handleValueChange} filterType={ft ?? "String"} type={vlt != null ? tr! : { name: "string" }} formatText={format} unitText={unit} />
